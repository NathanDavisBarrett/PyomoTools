...
class _Formulation(pmo.block, ABC):
    ...
    def __init__(self,variableNames:List[str], variableInfo:Dict[str, Tuple[Union[pmo.variable, pmo.expression], Tuple[float, float]]]):
        super().__init__()
        ...
        for i,name in enumerate(self.variableNames):
            ...
            bounds = list(bounds)
            if len(bounds) != 2:
                raise ValueError(f"Bounds for variable '{name}' must be a tuple of length 2.")
            if bounds[0] is None:
                if hasattr(self.originalVariables[i], 'lb') and self.originalVariables[i].lb is not None:
                    bounds[0] = self.originalVariables[i].lb
            if bounds[1] is None:
                if hasattr(self.originalVariables[i], 'ub') and self.originalVariables[i].ub is not None:
    ...
    def registerConstraint(self, func:Callable[[Any], Union[pmo.expression, bool]], name:str=None):
        """
    ...
    def Plot_Scatter(self, numSamples=10000, color='blue', dotSize=10):
        ...
        feasibleMask = np.array([self._testFeasibility(point) for point in variableSamples])
        
        feasiblePoints = variableSamples[feasibleMask]
        if len(self.variableNames) in [0, 1]:
            raise NotImplementedError("Plotting is not available for this formulation due to insufficient number of variables.")
        if len(self.variableNames) == 2:
            ax.scatter(feasiblePoints[:, 0], feasiblePoints[:, 1], s=dotSize, color=color)
            ax.set_xlabel(self.variableNames[0])
            ax.set_ylabel(self.variableNames[1])
        elif len(self.variableNames) == 3:
            ax.scatter(feasiblePoints[:, 0], feasiblePoints[:, 1], feasiblePoints[:, 2], s=dotSize, color=color)
            ax.set_xlabel(self.variableNames[0])
            ax.set_ylabel(self.variableNames[1])
            ...
            def update_plot(slider_values):
                ...
                for point in feasiblePoints:
                    ...
                    if matches_sliders:
                        filtered_points.append(point[:3])  # Only first 3 dimensions for plotting
                ...
                slider_info = ", ".join([f"{self.variableNames[i+3]}={val:.2f}" 
            ...
            for i, slider_val in enumerate(initial_slider_values):
                ...
                if samplesPer < 1:
                    ...
                    slider = Slider(slider_ax, var_name, low, high, valinit=slider_val, valstep=sampleRanges[i+3])
