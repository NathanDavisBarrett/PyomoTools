...
class ConstrainContiguousBinarySeries(pmo.block):
    ...
    * A series of binary variables representing whether or not a machine is on at each time step t. The machine can only turn on and off one time in the series.
    ...
    """
    def __init__(self, X: Iterable[pmo.variable],minimumLength: int = 1,maximumLength: int = None):
        """
        Parameters
        ----------
        ...
        self.StartIndex = pmo.variable(domain=pmo.Integers, lb=minIndex, ub=maxIndex, value=minIndex)
        self.EndIndex = pmo.variable(domain=pmo.Integers, lb=minIndex, ub=maxIndex, value=maxIndex)

        self.MinimumLengthConstraint = pmo.constraint(expr=self.StartIndex <= self.EndIndex - minimumLength + 1)
        if maximumLength is not None:
            self.MaximumLengthConstraint = pmo.constraint(expr=self.EndIndex <= self.StartIndex + maximumLength - 1)
        ...
        self.AfterStartDefinition = pmo.block_list([ #IsAfterStart[i] = 1 if StartIndex <= i
            ConditionalLeq(
                A = self.StartIndex,
                alpha = i,
        ...
        self.BeforeEndDefinition = pmo.block_list([ #IsBeforeEnd[i] = 1 if EndIndex >= i
            ConditionalGeq(
                A = self.EndIndex,
                alpha = i,
