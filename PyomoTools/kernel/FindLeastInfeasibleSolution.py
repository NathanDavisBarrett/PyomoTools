...
def AugmentModel(model:pmo.block):
    ...
    for c in model.children():
        ...
        if isinstance(c,(pmo.variable_list,pmo.variable_tuple)):
            lowerBoundConstrs.extend([pmo.constraint(lowerBound(c[i])) for i in range(len(c))])
            upperBoundConstrs.extend([pmo.constraint(upperBound(c[i])) for i in range(len(c))])
...
def FindLeastInfeasibleSolution(originalModel:pmo.block,solver,leastInfeasibleDefinition:LeastInfeasibleDefinition=LeastInfeasibleDefinition.L1_Norm,solver_args:tuple=(),solver_kwargs:dict={},**kwargs):
    """
    ...
    **kwargs: dict
    ...
    if leastInfeasibleDefinition == LeastInfeasibleDefinition.L1_Norm:
        augmentedModel.LEAST_INFEASIBLE_L1_OBJ = pmo.objective(sum(slackVars),sense=pmo.minimize)
    elif leastInfeasibleDefinition == LeastInfeasibleDefinition.L2_Norm:
        augmentedModel.LEAST_INFEASIBLE_L2_OBJ = pmo.objective(sum(s**2 for s in slackVars),sense=pmo.minimize)
    ...
    if leastInfeasibleDefinition == LeastInfeasibleDefinition.Sequential:
        ...
        augmentedModel.LEAST_INFEASIBLE_L1_OBJ = pmo.Objective(expr=sum(slackVars),sense=pmo.minimize)
