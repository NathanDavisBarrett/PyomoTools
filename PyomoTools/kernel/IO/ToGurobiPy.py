    ...
    for c in model.children():
        if isinstance(c,pmo.variable):
            ...
            vr = grbModel.addVars(
                list(c.keys()),
                name=nameSpacer + c.local_name,
                lb={k: (v.lb if v.lb is not None else -gurobipy.GRB.INFINITY) for k, v in c.items()},
                ub={k: (v.ub if v.ub is not None else gurobipy.GRB.INFINITY) for k, v in c.items()},
            ...
            subMap = _CopyVariables(grbModel, c, nameSpacer + nameSpaceDelimiter + c.local_name)
            variableMap.update(subMap)

        elif isinstance(c, (pmo.block_list, pmo.block_tuple)):
            for i, b in enumerate(c):
                subMap = _CopyVariables(grbModel, b, nameSpacer + nameSpaceDelimiter + f"[{i}]" + b.local_name)
                variableMap.update(subMap)
        elif isinstance(c, pmo.block_dict):
            for k, b in c.items():
                subMap = _CopyVariables(grbModel, b, nameSpacer + nameSpaceDelimiter + f"[{k}]" + b.local_name)
                variableMap.update(subMap)
    return variableMap
...
def _ConvertRelational(c:pmo.constraint, varMap:dict):
    ...
    if ub is not None and lb is not None:
        assert np.allclose(ub, lb), "Upper and lower bounds must be equal for equality constraints."
        ...
        raise ValueError("At least one of upper or lower bounds must be specified for a constraint.")
    return expr
...
def _CopyConstraints(grbModel:gurobipy.Model, model:pmo.block, varMap, nameSpacer=""):
    for c in model.children():
        if isinstance(c,pmo.constraint):
            ...
            _CopyConstraints(grbModel, c, varMap, nameSpacer + nameSpaceDelimiter + c.local_name)
        elif isinstance(c, (pmo.block_list, pmo.block_tuple)):
            for i, b in enumerate(c):
                _CopyConstraints(grbModel, b, varMap, nameSpacer + nameSpaceDelimiter + f"[{i}]" + b.local_name)
        elif isinstance(c, pmo.block_dict):
            for k, b in c.items():
                _CopyConstraints(grbModel, b, varMap, nameSpacer + nameSpaceDelimiter + f"[{k}]" + b.local_name)

def _CopyObjectives(grbModel:gurobipy.Model, model:pmo.block, varMap:dict, nameSpacer=""):
    for c in model.children():
        if isinstance(c,pmo.objective):
            ...
            grbModel.setObjective(
                ...
                sense=gurobipy.GRB.MINIMIZE if c.sense == pmo.minimize else gurobipy.GRB.MAXIMIZE
            )
        elif isinstance(c, (pmo.objective_list, pmo.objective_tuple)):
            raise NotImplementedError("Objective lists and tuples are not supported in GurobiPy conversion.")
        elif isinstance(c, pmo.objective_dict):
            raise NotImplementedError("Objective dictionaries are not supported in GurobiPy conversion.")

        elif isinstance(c,pmo.block):
            _CopyObjectives(grbModel, c, varMap, nameSpacer + nameSpaceDelimiter + c.local_name)
        elif isinstance(c, (pmo.block_list, pmo.block_tuple)):
            for i, b in enumerate(c):
                _CopyObjectives(grbModel, b, varMap, nameSpacer + nameSpaceDelimiter + f"[{i}]" + b.local_name)
        elif isinstance(c, pmo.block_dict):
            for k, b in c.items():
                _CopyObjectives(grbModel, b, varMap, nameSpacer + nameSpaceDelimiter + f"[{k}]" + b.local_name)
