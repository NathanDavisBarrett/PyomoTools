...
def ModelToDict(model:pmo.block,reprKeys:bool=True):
    ...
    for c in model.children():
        ...
        if isinstance(c,(pmo.variable_list,pmo.variable_tuple)):
            ...
            dct[cName] = {FormatKey(repr(k) if reprKeys else k):pmo.value(c[k],exception=False) for k in c}
            ...
            dct[cName] = {FormatKey(repr(k) if reprKeys else k):ModelToDict(c[k]) for k in c}
