...
class MatrixRepresentation:
    ...
    def __str__(self):
        ...
        ans = []
        ans.append(f"min [" + ",".join(self.c.astype(str)) + "]^T [" + ",".join(varNames) + "]")
        ans.append(f"subject to:")
    ...
    def vertices(self):
        ...
        if dim > numConstr:
            return [] #There cannot be any vertices if there are more variables than constraints.
        ...
        try:
            ...
            if self.vertices.shape[1] == 3:  # 3D points
                ...
                for simplex in hull.simplices:
                    ...
                    poly = Poly3DCollection([verts], facecolors='lightblue', 
                                            linewidths=1, edgecolors='blue', alpha=0.6)
            ...
            for i, vcomb in enumerate(comb):
                ...
                poly = Poly3DCollection([verts], facecolors='lightblue', 
                                        linewidths=1, edgecolors='blue', alpha=0.4)
                ax.add_collection3d(poly)

        ax.scatter(self.vertices[:, 0], self.vertices[:, 1], 
                            self.vertices[:, 2], c='red', s=50, alpha=0.8)
    ...
    def _PlotHigherDim(self, ax=None):
        ...
        if ax is not None:
            raise ValueError("_PlotHigherDim does not support external axes due to widget requirements")
        ...
        x_checks = CheckButtons(ax_x_check, var_names, 
                               [i == x_axis_var for i in range(numVar)])
        y_checks = CheckButtons(ax_y_check, var_names,
                               [i == y_axis_var for i in range(numVar)])
        z_checks = CheckButtons(ax_z_check, var_names,
                               [i == z_axis_var for i in range(numVar)])
        ...
        def create_sliders():
            ...
            for i, var_idx in enumerate(slider_vars):
                ...
                slider = Slider(ax_slider, var_names[var_idx], 
                              var_min, var_max, 
        ...
        def update_plot():
            ...
            if len(self.vertices) == 0:
                ax_3d.text(0.5, 0.5, 0.5, 'No feasible vertices found', 
            ...
            for vertex in self.vertices:
                projected_vertex = [vertex[x_axis_var], 
                                  vertex[y_axis_var], 
                                  vertex[z_axis_var]]
            ...
            if len(unique_vertices) > 0:
                ...
                ax_3d.scatter(unique_vertices[:, 0], unique_vertices[:, 1], 
                            unique_vertices[:, 2], c='red', s=50, alpha=0.8)
            ...
            if len(unique_vertices) > 0:
                ...
                ax_3d.set_xlim([np.min(unique_vertices[:, 0]) - margin * x_range, 
                               np.max(unique_vertices[:, 0]) + margin * x_range])
                ax_3d.set_ylim([np.min(unique_vertices[:, 1]) - margin * y_range, 
                               np.max(unique_vertices[:, 1]) + margin * y_range])
                ax_3d.set_zlim([np.min(unique_vertices[:, 2]) - margin * z_range, 
            ...
            if info_text:
                ax_3d.text2D(0.02, 0.98, "Fixed values:\n" + "\n".join(info_text), 
            ...
            for newEntries,newConst in results:
                ...
                if numNewEntries > 0:
                    if baseResult is not None:
                        raise Exception("Bilinear term detected! Currently, MatrixRepresentation can only handle linear models.")
