...
def PlotProb(A,b,c,xfeas=None):
    ...
    plt.imshow(mask.astype(int), extent=(-2, 2, -2, 2), origin="lower", cmap="Greys", alpha=0.3)
...
def test_BigRandom():
    ...
    model.obj = pyo.Objective(expr=sum(c[j] * model.x[j] for j in model.N),sense=pyo.minimize)
