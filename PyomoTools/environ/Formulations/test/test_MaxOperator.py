...
def indexed_Base(includeBinary,fullModel):
    ...
    if fullModel:
        model.obj = pyo.Objective(expr=sum(model.A[i1,i2] for i1,i2 in model.TestSet*model.TestSet),sense=pyo.maximize)
    ...
    model.obj1 = pyo.Objective(expr=sum(model.A[i1,i2] for i1,i2 in model.TestSet*model.TestSet),sense=pyo.minimize)
    solver.solve(model)
