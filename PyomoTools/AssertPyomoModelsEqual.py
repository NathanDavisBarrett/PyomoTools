...
def AssertPyomoModelsEqual(model1:pyo.ConcreteModel,model2:pyo.ConcreteModel,rtol=1e-5,atol=1e-8,error=False,warning=False) -> bool:
    """
    A function to determine if two pyomo models contain equal solutions.
    ...
    if variableNames1 != variableNames2:
        if error or warning:
            ...
            message = "The following variables are present in model1 but not model2:\n{}\n\nThe following variables are present in model2 but not model1:\n{}".format(variablesIn1ButNot2,variablesIn2ButNot1)
            if error:
    ...
    for v in variableNames1:
        ...
        if v1Indices != v2Indices:
            if error or warning:
                message = f"The indices for variable \"{v}\" are not consistent between the two models.\nModel 1 Indices: {v1Indices}\nModel 2 Indices: {v2Indices}"
                if error:
                    raise Exception(message)
                if warning:
                    warnings.warn(message)
        ...
        for index in v1:
            ...
            if val1 is None or val2 is None:
                if val1 is not None or val2 is not None:
                    if error or warning:
                        message = "The values for {}[{}] do not match! model1: {}, model2: {}".format(v,index,val1,val2)
                        if error:
                            raise Exception(message)
                        if warning:
                            warnings.warn(message)
