    ...
    **kwargs: dict
    ...
    for var in augmentedModel.component_objects(pyo.Var, active=True):
        ...
        if isIndexed:
            ...
            setattr(augmentedModel,f"{var}_LOWER_BOUND",pyo.Constraint(var.index_set(),rule=lowerBound))
            setattr(augmentedModel,f"{var}_UPPER_BOUND",pyo.Constraint(var.index_set(),rule=upperBound))
            
        else:
            bounds = var.bounds
            if bounds[0] is not None:
                constrName = f"{var}_LOWER_BOUND"
                setattr(augmentedModel,constrName,pyo.Constraint(expr=bounds[0]<=var))
                var.setlb(None)
            if bounds[1] is not None:
                constrName = f"{var}_UPPER_BOUND"
                setattr(augmentedModel,constrName,pyo.Constraint(expr=bounds[1]>=var))
                var.setub(None)



    #Step 2, copy over all constraints, relaxing each one using additional slack variables.
    constrNames = [str(constr) for constr in augmentedModel.component_objects(pyo.Constraint, active=True)]
    for constrName in constrNames:
        ...
        if isIndexed:
            ...
            setattr(augmentedModel,slackVarName,pyo.Var(indexSet,domain=pyo.NonNegativeReals))
        ...
        if isIndexed:
            ...
            setattr(augmentedModel,lowerConstrName,pyo.Constraint(indexSet,rule=lowerConstr))
            setattr(augmentedModel,upperConstrName,pyo.Constraint(indexSet,rule=upperConstr))
                
        else:
            body = constr.body
            if constr.lower is not None:
                lowerConstrName = f"{constrName}_LOWER_CONSTR"
                setattr(augmentedModel,lowerConstrName,pyo.Constraint(expr=constr.lower - slackVar <= body))
            if constr.upper is not None:
                upperConstrName = f"{constrName}_UPPER_CONSTR"
                setattr(augmentedModel,upperConstrName,pyo.Constraint(expr= body <= constr.upper + slackVar))
