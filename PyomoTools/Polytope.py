...
class Polytope:
    ...
    def __init__(self,model:pyo.ConcreteModel,vars:list):
        ...
        A,b,c,d,inequalityIndices,equalityIndices = self.vecRep.Generate_Matrix_Representation()
        ...
        relevantInequalityIndices = np.intersect1d(inequalityIndices,relevantConstrIndices)
        ...
        self.relevantConstrNames = np.hstack([relevantInequalityNames,relevantEqualityNames,relevantEqualityNames])
        ...
        targetVarIndices = np.array([np.where(relevantVarNames == str(v))[0] for v in vars]).flatten()
    ...
    def Plot(self):
        ...
        for i,v in enumerate(self.knownVarNames):
            var = self.vecRep.VAR_VEC[self.vecRep.VAR_VEC_INV[v]]
            bounds = var.bounds
            sliderAx = self.fig.add_axes([0.05,i/len(self.knownVarNames),0.1,1/len(self.knownVarNames)])
        ...
        if self.numDim == 2:
            self.vertexScatter = self.ax.scatter([0,],[0,],marker='o')

            for i in range(len(self.relevantConstrNames)):
                self.constrPlots[i], = self.ax.plot([0,],[0,],label=self.relevantConstrNames[i])
        else:
            for i in range(len(self.relevantConstrNames)):
                junk = np.array([0,1])
                junkX,junkY = np.meshgrid(junk,junk)
                self.constrPlots[i] = self.ax.plot_surface(junkX,junkX,junkX,alpha=0.5,color=self.defaultColorCycle[i])

                for ip in range(i+1,len(self.relevantConstrNames)):
                    self.constrIntersectLines[i,ip], = self.ax.plot([0,],[0,],[0,],'k')

                self.ax.plot([0,],[0,],[0,],color=self.defaultColorCycle[i],alpha=0.5,label=self.relevantConstrNames[i])
    ...
    def _UpdatePlot_2D(self,*junk):
        ...
        for i in range(len(self.relevantConstrNames)):
            ...
            fesibleMask = np.logical_and(fesibleMask,self.A_final[i,0] * X + self.A_final[i,1] * Y <= b[i])
    ...
    def _GetConstrData_3D(self,i,b,mins,maxs):
        ...
        if Ai[axisRanking[2]] == 0:
            if Ai[axisRanking[1]] == 0:
                ...
                X1,X2 = np.meshgrid(np.array([mins[axisRanking[1]],maxs[axisRanking[1]]]), np.array([mins[axisRanking[2]],maxs[axisRanking[2]]]))
                X0 = np.full_like(X1,bi/Ai[axisRanking[0]])
            else:
                X0, X2 = np.meshgrid(np.array([mins[axisRanking[0]],maxs[axisRanking[0]]]), np.array([mins[axisRanking[2]],maxs[axisRanking[2]]]))
                X1 = (-X0*Ai[axisRanking[0]] + bi)/Ai[axisRanking[1]]
        else:
            X0,X1 = np.meshgrid(np.linspace(mins[axisRanking[0]],maxs[axisRanking[0]],2), np.linspace(mins[axisRanking[1]],maxs[axisRanking[1]],2))
            X2 = (-X0*Ai[axisRanking[0]] - X1*Ai[axisRanking[1]] + bi) / Ai[axisRanking[2]]
    ...
    def _UpdatePlot_3D(self,*junk):
        ...
        for i in range(len(self.relevantConstrNames)):
            ...
            self.constrPlots[i] = self.ax.plot_surface(Xi,Yi,Zi,alpha=0.5,color=self.defaultColorCycle[i])
            fesibleMask = np.logical_and(fesibleMask,self.A_final[i,0] * X + self.A_final[i,1] * Y + self.A_final[i,2] * Z <= b[i])
            ...
            for ip in range(i+1,len(self.relevantConstrNames)):
                ...
                if tmin < tmax:
                    self.constrIntersectLines[i,ip], = self.ax.plot(*[[start[i],stop[i]] for i in range(3)],'k')
                else:
        ...
        # vertices = GeneratePolytopeVertices(model,vars)

        # # THEN TAKE A LOOK AT MORALES ESPANAS "TIGHTER" POWER OUTPUT LIMIT CONSTR.

        # if len(vars) == 3:
        #     #This will be a 3D graph:
        #     faces = ConvexHull(vertices)
        ...
        #         poly = Poly3DCollection([vertices[indices,:]], alpha=0.5, linewidths=1, edgecolors='r')
        ...
        #     ax.set_zlabel(str(vars[2]))

        #     # Show the plot
        #     plt.show()
        # elif len(vars) == 2:
        #     #This will be a 2d plot
        #     faces = ConvexHull(vertices)
        ...
        #     # Plot the convex hull
        #     for indices in faces:
        #         ax.plot(vertices[indices, 0], vertices[indices, 1], 'k-')
        ...
        #     plt.show()
        # else:
        #     raise NotImplementedError(f"Cannot create a ploy of a polytope of dimension {len(vars)} at this time.")
