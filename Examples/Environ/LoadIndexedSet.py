...
from PyomoTools.environ import LoadIndexedSet
...
delta = {"AB": 15, "AC": 5, "BC": 4, "BD": 2, "CD": 10}
...
LoadIndexedSet(
    model, "setC_p", {p: [c for c in model.setC if p in c] for p in model.setP}
)
...
def NonTerminalTravelConstraint(model, p):
    return (
        sum(model.ExecuteTransition[c] for c in model.setC_p[p])
        == 2 * model.VisitPoint[p]
    )


model.NonTerminalTravelConstraint = pyo.Constraint(
    model.setP_NONTerminal, rule=NonTerminalTravelConstraint
)


def TerminalTravelConstraint(model, p):
    return (
        sum(model.ExecuteTransition[c] for c in model.setC_p[p]) == model.VisitPoint[p]
    )


model.TerminalTravelConstraint = pyo.Constraint(
    model.setP_Terminal, rule=TerminalTravelConstraint
)
...
model.Obj = pyo.Objective(
    expr=sum(delta[c] * model.ExecuteTransition[c] for c in model.setC)
)
